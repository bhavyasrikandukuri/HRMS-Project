...
//			    ps.setString(2, password);
	...
	@Override
	public String updateProjectCompletionStatus(String username, String status, String dateTime) throws EmployeeException {
		 String message="Please Enter a proper username";
		 
		 
		   try(Connection conn=DB_Connect.getConnection()){
		    	PreparedStatement ps =conn.prepareStatement(" INSERT INTO updateStatus(username,status,workingdDept, sdateTime) VALUES(?,?,(SELECT deptId FROM employee where username=?),?)");
